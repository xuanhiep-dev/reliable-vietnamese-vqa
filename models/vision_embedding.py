...
class ResnetExtractor(nn.Module):
    def __init__(self):
        ...
        self.transform = self.get_transforms(
            target_size=224, central_fraction=0.875)
    ...
    def forward(self, *images):
        images_transformed = torch.stack(
            [self.transform(image.convert('RGB')).to(self.device) for image in images])
...
class Blip2ViTExtractor(nn.Module):
    ...
    def forward(self, *images):
        ...
        image_features = self.model.extract_features(
            samples={"image": images}, mode="image").image_embeds
...
class EfficientnetExtractor(nn.Module):
    def __init__(self, model_name):
        ...
        self.model = EfficientNet.from_pretrained(
            model_name, advprop=True).to(self.device)
        self.pooling1 = nn.AdaptiveAvgPool2d((1, 32))
        self.pooling2 = nn.AdaptiveAvgPool2d((1, 768))
...
class Blip2EfficientExtractor(nn.Module):
    def __init__(self):
        ...
        # BLIP-2
        self.model_blip2 = registry.get_model_class(
            name="blip2_feature_extractor").from_pretrained(model_type="pretrain").to(self.device)
        if self.device == "cpu" or self.device == torch.device("cpu"):
            self.model_blip2 = self.model_blip2.float()
        self.model_blip2.eval()

        # Efficientnet
        self.model_efficientnet = EfficientNet.from_pretrained(
            'efficientnet-b7', advprop=True).to(self.device)
    ...
    def forward(self, images):
        # Extract global
        global_features = self.model_blip2.extract_features(
            samples={"image": images}, mode="image").image_embeds
        ...
        local_features = local_features.reshape(
            batch_size, local_features.shape[1], -1)
